...
export async function searchUrls(search) {
...
export async function writeUrls(anime, output, res, options) {
...
export async function clipboardUrls(anime, res, options) {
   const instance = new Vidstreaming(res, options);
   loading.start();
...
export async function downloadUrls(anime, output, res, options) {
   const instance = new Vidstreaming(res, options);
   loading.start();
   let bar;
   ...
   instance.on('queue', (i, f, l) => {
      ...
      bar = new ProgressBar(
         `DL - [:bar] ${i.cur}/${i.total} :percent :size/${l.human}`,
         {
            width: 30,
            complete: '#',
            incomplete: ' ',
