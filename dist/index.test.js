...
describe('Vidstreaming API test...', () => {
  test('Basic episode fetching...', async done => {
    const results = await new _vidstreaming.default('5-toubun').episodes(false);
  ...
  test('Basic async episode fetching...', async done => {
    const results = await new _vidstreaming.default('5-toubun', null, {
      async: true
    }).episodes(false);
    expect.hasAssertions();
    expect(results).toEqual(expect.arrayContaining([expect.objectContaining({
  ...
  test('Error testing...', done => {
    const vid = new _vidstreaming.default('some anime that doesnt exist');
    vid.episodes(false);
